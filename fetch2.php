...
if(isset($_POST["state_id"]) && !empty($_POST["state_id"])){ 
    $query = "SELECT * FROM city WHERE city_state_id =".$_POST["state_id"]." AND assign_status='0' ORDER BY city_name ASC"; 
    ...
    if($result->num_rows > 0){ 
        ...
        while($row = $result->fetch_assoc()){  
            echo '<option selected value="'.$row['Pincode'].'">'.$row['Location'].'('.$row['Pincode'].')</option>'; 
        } 
    }else{ 
